...
//float Tx_min
int wait_time = 5;
float checking_interval = 1;
boolean isChhecking=false;
...
void loop() {
  ...
  { 
    if (timeInterval % (200000 * wait_time) == 0)
    {
      for (int i = 0; i < 200000; i++)
      {
            numUsers = WiFi.softAPgetStationNum();
            if (numUsers > 1) {
              currPower = TxMinPower;
              WiFi.setOutputPower(currPower);
              break;
            }
            else currPower = powerLevel[1];
      }
      WiFi.setOutputPower(currPower);
    }
